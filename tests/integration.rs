...
async fn cleanup(client: &containerd_manager::Client, id: &ContainerId) {
    let _ = client.delete_container(id, TIMEOUT).await;
}

async fn cleanup_by_name(client: &containerd_manager::Client, name: &str) {
    if let Ok(id) = client.resolve_name(name).await {
        let _ = client.delete_container(&id, TIMEOUT).await;
    }
}
...
async fn test_create_container() {
    let client = connect();
    let name = "create-test";

    cleanup_by_name(&client, name).await;
    client.pull_image(ALPINE).await.unwrap();
    ...
    let opts = CreateContainerOpts::builder()
        ...
        .build();
    client
        .create_container(name, ALPINE, opts.clone())
        .await
        .unwrap();

    // Duplicate name should fail.
    let err = client.create_container(name, ALPINE, opts).await;
    assert!(
...
async fn test_start_and_delete_container() {
    ...
    let opts = CreateContainerOpts::builder()
        ...
        .build();
    let id = client.create_container(name, ALPINE, opts).await.unwrap();
...
async fn test_inspect_container() {
    ...
    let nonexistent = ContainerId::from_existing("doesnotexistxyz").unwrap();
...
async fn test_list_containers() {
    ...
    cleanup_by_name(&client, name1).await;
    cleanup_by_name(&client, name2).await;
    client.pull_image(ALPINE).await.unwrap();

    let opts1 = CreateContainerOpts::builder()
        .label("app", "frontend")
        .label("list-group", "e2e")
        .build();
    let id1 = client.create_container(name1, ALPINE, opts1).await.unwrap();

    let opts2 = CreateContainerOpts::builder()
        .label("app", "backend")
        .label("list-group", "e2e")
        .build();
    let id2 = client.create_container(name2, ALPINE, opts2).await.unwrap();
...
async fn test_container_logs() {
    ...
    let opts = CreateContainerOpts::builder()
        .label("test", "logs")
        .cmd(["sh", "-c", "echo hello-from-logs && sleep 300"])
        .build();
    let id = client.create_container(name, BUSYBOX, opts).await.unwrap();
...
async fn test_wait_for_exit() {
    ...
    let opts = CreateContainerOpts::builder()
        .label("test", "wait-exit")
        .cmd(["sh", "-c", "sleep 0.2; exit 42"])
        .build();
    let id = client.create_container(name, BUSYBOX, opts).await.unwrap();
    client.start_container(&id).await.unwrap();
...
async fn test_wait_for_exit_timeout() {
    let client = connect();
    let name = "wait-exit-timeout-test";
...
async fn test_container_logs_stream_until_exit() {
    let client = connect();
    let name = "logs-stream-until-exit-test";
    ...
    let opts = CreateContainerOpts::builder()
        .cmd(["sh", "-c", "echo one; echo two; echo three; sleep 0.5"])
        .build();
    let id = client.create_container(name, BUSYBOX, opts).await.unwrap();
    client.start_container(&id).await.unwrap();
...
async fn test_port_forward() {
    ...
    let client = connect();
    // start_port_forward doesn't require the container to exist; the ID is
    // only used as a key for managed-handle tracking.
    let id = ContainerId::from_existing("portfwdtestfake").unwrap();
...
async fn test_start_nonexistent_container_errors() {
    let client = connect();
    let result = client
        .start_container(&ContainerId::from_existing("doesnotexistxyz").unwrap())
        .await;
    assert!(
