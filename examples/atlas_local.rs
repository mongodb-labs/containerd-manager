...
use containerd_manager::{
    CreateContainerOpts, LogStream, NetworkMode, NetworkOpts, ReadinessStrategy, TaskStatus,
};

#[tokio::main(flavor = "current_thread")]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let image = "docker.io/mongodb/mongodb-atlas-local:latest";
    let name = "atlas-local-1";
    ...
    let container_id = client.create_container(name, image, opts).await?;
