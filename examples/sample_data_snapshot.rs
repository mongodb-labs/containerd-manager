...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let opts = CreateContainerOpts::builder()
        .env("MONGODB_LOAD_SAMPLE_DATA", "true")
        .build();
    let primed = client.create_container(primed_name, IMAGE, opts).await?;
    ...
    let restored = client.restore_container(SNAPSHOT, restored_name).await?;
