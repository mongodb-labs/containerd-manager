...
pub(crate) fn prestart_hook_script(
    container_ip: &str,
    container_id: &str,
    namespace: &str,
    ...
    let nerdctl_compat =
        nerdctl_network_config_script(namespace, container_id, host_container_pairs);
    format!(
        r#"
STATE=$(cat)
...
/// Writes nerdctl's per-container `network-config.json` so `nerdctl ps`
/// shows the PORTS column without the legacy-label warning introduced by
/// containerd/nerdctl#4290. Path layout:
///
///   /var/lib/nerdctl/<addrHash>/containers/<namespace>/<container_id>/network-config.json
///
/// where `<addrHash>` = `sha256("/run/containerd/containerd.sock")[:8]` =
/// `1935db59`. This matches the default rootful nerdctl install with the
/// standard containerd socket. Non-default socket paths or rootless nerdctl
/// won't find the file, but the legacy label fallback was removed so they
/// just see an empty PORTS column instead of a warning.
fn nerdctl_network_config_script(
    namespace: &str,
    container_id: &str,
    host_container_pairs: &[(u16, u16)],
) -> String {
    if host_container_pairs.is_empty() {
        return String::new();
    }
    let entries: Vec<String> = host_container_pairs
        .iter()
        .map(|&(hp, cp)| {
            format!(
                r#"{{"HostPort":{hp},"ContainerPort":{cp},"Protocol":"tcp","HostIP":"0.0.0.0"}}"#
            )
        })
        .collect();
    let json = format!(r#"{{"portMappings":[{}]}}"#, entries.join(","));
    let dir = format!("/var/lib/nerdctl/1935db59/containers/{namespace}/{container_id}");
    format!(
        r#"
# nerdctl compat: write port-mapping config so `nerdctl ps` shows PORTS.
NERDCTL_DIR="{dir}"
mkdir -p "$NERDCTL_DIR" 2>/dev/null && chmod 700 "$NERDCTL_DIR" 2>/dev/null || true
cat > "$NERDCTL_DIR/network-config.json" 2>/dev/null <<'NERDCTL_EOF' || true
{json}
NERDCTL_EOF
chmod 600 "$NERDCTL_DIR/network-config.json" 2>/dev/null || true
"#
    )
}
...
/// veth.
pub(crate) fn poststop_hook_script(
    container_id: &str,
    namespace: &str,
    ports: &[(u16, u16)],
) -> String {
    ...
    // Clean up nerdctl compat dir (best-effort; ignore if absent).
    script.push_str(&format!(
        "rm -rf \"/var/lib/nerdctl/1935db59/containers/{namespace}/{container_id}\" 2>/dev/null || true\n"
    ));
    script
}
...
mod tests {
    ...
    #[test]
    fn prestart_writes_nerdctl_network_config_when_ports_set() {
        let script = prestart_hook_script("10.88.1.5", "cid", "demo-ns", &[(50123, 27017)]);
        // Path matches nerdctl's rootful dataStore with the default socket
        // addr hash (1935db59 = sha256("/run/containerd/containerd.sock")[:8]).
        assert!(script.contains("/var/lib/nerdctl/1935db59/containers/demo-ns/cid"));
        assert!(script.contains(r#""HostPort":50123"#));
        assert!(script.contains(r#""ContainerPort":27017"#));
    }

    #[test]
    fn prestart_omits_nerdctl_section_with_no_ports() {
        let script = prestart_hook_script("10.88.1.5", "cid", "demo-ns", &[]);
        assert!(!script.contains("nerdctl"));
    }

    #[test]
    fn poststop_removes_nerdctl_config_dir() {
        let script = poststop_hook_script("cid", "demo-ns", &[]);
        assert!(script.contains("rm -rf \"/var/lib/nerdctl/1935db59/containers/demo-ns/cid\""));
    }
    ...
    fn poststop_kills_each_socat_then_deletes_veth() {
        let script = poststop_hook_script("cid", "test-ns", &[(27017, 50001), (8080, 50002)]);
