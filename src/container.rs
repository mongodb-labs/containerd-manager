...
/// (prepare) and remove_container (remove).
...
fn build_bridge_hooks(
    ...
    let poststop = sh_hook(&crate::bridge::poststop_hook_script(container_id, ports))?;
...
pub(crate) async fn get_raw_image_config(client: &Client, image: &str) -> Result<Vec<u8>> {
    ...
    let metadata =
        get_image_metadata(client, &mut content_client, &manifest_digest, client.namespace())
            .await?;
    Ok(metadata.raw_config.clone())
}

/// Creates the container record only. The task is started separately via
/// [`crate::task::start_container`].
pub(crate) async fn create_container(
    ...
    let (snapshot_key, we_prepared) = match opts_owned.from_existing_snapshot.as_deref() {
        ...
        None => {
            let key = snapshot_key_for(container_id);
            ...
            client.snapshots().prepare(prepare_req).await.map_err(|e| {
                if e.code() == containerd_client::tonic::Code::AlreadyExists {
                    Error::ContainerAlreadyExists(container_id.to_string())
                } else {
    ...
    let hooks: Option<Hooks> = if let Some((ref ip, ref ports)) = bridge_config {
        Some(build_bridge_hooks(container_id, ip, ports)?)
    } else {
        None
    };
    ...
    // Port-binding labels: bridge mode uses the freshly-allocated host ports;
    // host/isolated mode passes through the caller's host_port directly.
    let port_bindings_for_labels: &[(u16, u16)] = match bridge_config {
        Some((ref ip, ref ports)) => {
            labels.insert(BRIDGE_IP_LABEL.to_string(), ip.clone());
            // Bridge emits (container_port, host_port); flip to match the
            // (host_port, container_port) convention used in labels.
        ...
        None => &opts.port_bindings,
    };
    for &(host_port, container_port) in port_bindings_for_labels {
        labels.insert(
            format!("{}{}", PORT_BINDING_LABEL_PREFIX, container_port),
            host_port.to_string(),
        );
...
mod tests {
    ...
    fn build_mounts_creates_oci_mounts() {
        let opts = CreateContainerOpts::builder()
            ...
            .mount_ro("/host/config", "/container/config").build();
    ...
    fn build_args_cmd_override_replaces_image_args() {
        let config = test_image_config_with_entrypoint();
        let opts = CreateContainerOpts::builder().cmd(["sleep", "infinity"]).build();
    ...
    fn build_args_cmd_override_on_empty_image() {
        let config = test_image_config();
        let opts = CreateContainerOpts::builder().cmd(["/bin/my-app", "--verbose"]).build();
        let args = build_args(&config, &opts);
    ...
    fn build_merged_env_combines_image_and_opts() {
        ...
        assert_eq!(path_entries.len(), 1, "expected exactly one PATH= entry, got {:?}", path_entries);
    ...
    fn build_oci_spec_uses_image_entrypoint() {
        let config = test_image_config_with_entrypoint();
        let spec = build_oci_spec(&config, &CreateContainerOpts::default(), None).expect("should build spec");
    ...
    fn create_container_opts_network_mode_builder() {
        let opts = CreateContainerOpts::builder()
            .network(NetworkOpts { mode: NetworkMode::Isolated })
            .build();
