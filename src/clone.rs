...
pub(crate) async fn clone_container(
    ...
    tracing::info!(src = %src, dst = %dst_name, restart_src = opts.restart_src, "clone_container: begin");
    ...
    let result = clone_inner(client, src, dst_name, &src_info, &opts).await;
    ...
    let dst_id = result?;
    tracing::info!(src = %src, dst = %dst_name, dst_id = %dst_id, "clone_container: done");
    Ok(dst_id)
}
...
async fn clone_inner(
    ...
    // Mint the dst ID upfront so the snapshot key matches the container
    // record we'll create below.
    let dst_id = ContainerId::generate();
    let dst_snapshot_key = crate::container::snapshot_key_for(dst_id.as_str());
    ...
    prepare_clone_snapshot(client, &snapshotter, &dst_snapshot_key, &base_key, dst_name).await?;

    // Clean up the orphan snapshot if container record creation fails, else
    // retries hit AlreadyExists on the snapshotter.
    if let Err(e) = create_clone_record(
        client,
        dst_id.clone(),
        dst_name,
        src_info,
    ...
    .await
    {
        tracing::warn!(dst = %dst_name, error = %e, "clone_container: dst create failed, removing orphan snapshot");
...
async fn create_clone_record(
    ...
    crate::container::create_container_with_id(
        client,
        dst_id,
        dst_name,
        &src_info.image,
