...
impl LogFollower {
    ...
    pub(crate) fn start_with_stop(events_path: PathBuf, stop: oneshot::Receiver<()>) -> Self {
...
mod tests {
    ...
    // Inode-based rotation detection is Unix-only (Windows fakes inode = 0).
    #[cfg(unix)]
    #[tokio::test]
    ...
    async fn follower_streams_via_stream_trait() {
        ...
        let entry =
            std::future::poll_fn(
                |cx: &mut Context<'_>| match Pin::new(&mut f).poll_next(cx) {
