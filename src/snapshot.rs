...
pub(crate) async fn restore_container(
    ...
    tracing::info!(snapshot = %snapshot_name, dst = %dst_name, "restore_container: begin");
    ...
    snap.prepare(prep_req).await.map_err(|e| {
        if e.code() == Code::AlreadyExists {
            Error::ContainerAlreadyExists(dst_name.to_string())
        } else {
    ...
    match crate::container::create_container_with_id(
        client,
        dst_id.clone(),
        dst_name,
        &image,
    ...
    {
        Ok(id) => {
            tracing::info!(snapshot = %snapshot_name, dst = %dst_name, dst_id = %id, "restore_container: done");
            Ok(id)
        }
        Err(e) => {
            tracing::warn!(
...
pub(crate) async fn reset_to_snapshot(
    ...
    let snapshot_image =
        stat_managed_snapshot(client, &mut snap, &snapshotter, snapshot_name).await?;
    ...
    if let Err(e) =
        crate::task::stop_container(client, container_id.as_str(), SNAPSHOT_OP_STOP_TIMEOUT).await
    {
        tracing::debug!(
            container = %container_id,
            error = %e,
