...
async fn tailer_loop(
    ...
    loop {
        let r1 = drain_into_events(&stdout_path, LogStream::Stdout, &mut stdout_state, &events_path).await;
        let r2 = drain_into_events(&stderr_path, LogStream::Stderr, &mut stderr_state, &events_path).await;
...
async fn drain_into_events(
    ...
    let file = state.file.as_mut().expect("file present after ensure_file_open");
...
mod tests {
    ...
    async fn stop_drains_pending_writes() {
        ...
        let data_concat: Vec<u8> = entries
            ...
            .collect();
        assert!(data_concat.windows(b"first-line\n".len())
                .any(|w| w == b"first-line\n"));
        assert!(data_concat.windows(b"second-line\n".len())
