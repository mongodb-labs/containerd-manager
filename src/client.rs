...
impl Client {
    ...
    /// Creates a fresh container with the given human name. Returns the
    /// freshly-minted opaque [`ContainerId`] hash. The same `name` can be
    /// reused after `delete_container` — each create yields a new
    /// `ContainerId`. `name` is validated against containerd's identifier
    /// rules (alphanumeric + `-_.`, 1-76 chars).
    #[tracing::instrument(skip(self, opts), fields(namespace = %self.namespace, name = %name))]
    pub async fn create_container(
        ...
        crate::container::create_container(self, name, image, opts).await
    }

    /// Resolves a human container name to the underlying opaque
    /// [`ContainerId`] in this client's namespace. Errors with
    /// `ContainerNotFound` if no active container carries the name.
    pub async fn resolve_name(&self, name: &str) -> Result<ContainerId> {
        crate::inspect::resolve_name(self, name).await
    }

    /// Convenience: `resolve_name` + `inspect_container`.
    pub async fn inspect_by_name(&self, name: &str) -> Result<ContainerInfo> {
        let id = self.resolve_name(name).await?;
    ...
    pub async fn start_container(&self, container_id: &ContainerId) -> Result<TaskId> {
        ...
        // No auto-managed port forward. Bridge mode reachability is handled
        // by the OCI poststart-hook socat on both Linux native and macOS:
        //   - Linux native: socat binds host_port in the same namespace.
        //   - macOS + colima: socat binds host_port inside the VM; colima's
        //     VZ network exposes VM-loopback to the macOS host.
        // Userspace proxies here would intercept and break the colima VZ
        // bridge (clients see ECONNRESET because we'd forward to nothing).
        // Callers who need a custom forwarding setup can opt in via
        // [`start_managed_port_forward`](Self::start_managed_port_forward).
    ...
    /// commit cost.
    #[tracing::instrument(skip(self), fields(namespace = %self.namespace, src = %src, dst = %dst_name))]
    pub async fn clone_container(&self, src: &ContainerId, dst_name: &str) -> Result<ContainerId> {
        crate::clone::clone_container(self, src, dst_name, CloneContainerOpts::default()).await
    }
    ...
    pub async fn clone_container_with_opts(
        ...
        crate::clone::clone_container(self, src, dst_name, opts).await
    }
    ...
    pub async fn snapshot_container(&self, src: &ContainerId, name: &str) -> Result<()> {
        crate::snapshot::snapshot_container(self, src, name, SnapshotContainerOpts::default()).await
    }
    ...
    /// [`RestoreContainerOpts`]. Returns the new container's opaque ID.
    #[tracing::instrument(skip(self), fields(namespace = %self.namespace, snapshot = %snapshot_name, dst = %dst_name))]
    pub async fn restore_container(
        &self,
        snapshot_name: &str,
        dst_name: &str,
    ) -> Result<ContainerId> {
        crate::snapshot::restore_container(
            self,
            snapshot_name,
            dst_name,
            RestoreContainerOpts::default(),
        )
        .await
    }
    ...
    pub async fn restore_container_with_opts(
        ...
        crate::snapshot::restore_container(self, snapshot_name, dst_name, opts).await
    }
