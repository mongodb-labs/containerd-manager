...
/// Human-readable container name (the [`ContainerName`] the caller passed
/// at create time). Stored as a label so the containerd record's own `id`
/// stays the opaque hash. Resolution: name → id via list_containers filter.
pub(crate) const NAME_LABEL: &str = "containerd-manager.name";
...
mod tests {
    ...
    fn label_keys_pinned() {
        ...
        assert_eq!(NAME_LABEL, "containerd-manager.name");
    }
