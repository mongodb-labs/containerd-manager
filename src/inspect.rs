...
pub(crate) fn container_to_info(
    ...
    // `name` falls back to the ID if the label is missing (foreign
    // containers created via nerdctl/ctr won't have it). nerdctl's own
    // `nerdctl/name` is a secondary fallback.
    let name = labels
        .get(NAME_LABEL)
        .or_else(|| labels.get("nerdctl/name"))
        .cloned()
        .unwrap_or_else(|| container.id.clone());

    Ok(ContainerInfo {
        id: ContainerId::from_existing(container.id)?,
        name,
...
pub(crate) async fn inspect_container(
    ...
    let container_resp = client.containers_client().get(get_req).await.map_err(|e| {
...
/// Resolves a human container name to the underlying [`ContainerId`] hash
/// via containerd's label-filter list. Errors with `ContainerNotFound` if
/// no container in the current namespace carries the matching name label.
pub(crate) async fn resolve_name(client: &Client, name: &str) -> Result<ContainerId> {
    use containerd_client::services::v1::ListContainersRequest;
    let filter = format!("labels.\"{}\"=={}", NAME_LABEL, name);
    let resp = client
        .containers_client()
        .list(client.ns_req(ListContainersRequest {
            filters: vec![filter],
        }))
        .await
        .map_err(|e| crate::util::StatusExt::into_crate_error(e, "resolve_name"))?
        .into_inner();
    let mut containers = resp.containers;
    let first = containers
        .pop()
        .ok_or_else(|| Error::ContainerNotFound(name.to_string()))?;
    ContainerId::from_existing(first.id)
}
